...
class AffaireViewSet(viewsets.ModelViewSet):
    ...
    def change_statut(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            date_specifique = serializer.validated_data.get("dateChangement")
    ...
    def assigner_responsable(self, request, pk=None):
        """
        Assigne un responsable à une affaire
        """
        affaire = self.get_object()
        serializer = AssignerResponsableSerializer(data=request.data)
        ...
        if serializer.is_valid():
            ...
            try:
                nouveau_responsable = User.objects.get(pk=responsable_id)
                ...
                if not request.user.is_staff and not request.user.has_perm(
                    "affaires_app.assigner_responsable"
                ):
                    return Response(
                        {
                            "detail": "Vous n'avez pas la permission d'assigner un responsable."
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )

                # Effectuer l'assignation
                changed = affaire.assigner_responsable(
                ...
                if changed:
                    return Response(
                        {
                            "success": True,
                            "message": f"Le responsable a été changé pour {nouveau_responsable.username}",
                            "responsable": {
                                "id": nouveau_responsable.id,
                                "nom": nouveau_responsable.username,
                                "username": nouveau_responsable.username,
                                "email": nouveau_responsable.email,
                            },
                        }
                    )
                else:
                    return Response(
                        {
                            "success": False,
                            "message": "Aucun changement effectué, le responsable est déjà assigné",
                        }
                    )

            except User.DoesNotExist:
                return Response(
                    {"detail": "L'utilisateur spécifié n'existe pas."},
    ...
    def initData(self, request, pk=None):
        ...
        return Response(
            {"offres": offres_serializer.data, "users": user_serializer.data}
        )
    ...
    def generer_facture(self, request, pk=None):
        ...
        if not facture:
            return Response(
                {"success": False, "message": "Impossible de créer la facture"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def marquer_rapport_termine(self, request, pk=None):
        ...
        if not rapport_id:
            return Response(
                {"success": False, "message": "L'ID du rapport est requis"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"success": True, "message": "Rapport marqué comme terminé avec succès"}
            )
        except Rapport.DoesNotExist:
            return Response(
                {"success": False, "message": "Rapport non trouvé pour cette affaire"},
                status=status.HTTP_404_NOT_FOUND,
            )
    ...
    def dashboard(self, request):
        ...
        dernieres_affaires_serialized = AffaireSerializer(
            dernieres_affaires, many=True
        ).data

        # Affaires en retard
        affaires_en_retard = Affaire.objects.filter(
            statut="EN_COURS", date_fin_prevue__lt=now()
        ).order_by("date_fin_prevue")[:5]
        affaires_en_retard_serialized = AffaireSerializer(
            affaires_en_retard, many=True
        ).data

        # Résumé financier
        resume_financier = {
            "montant_total": Affaire.objects.aggregate(Sum("montant_total"))[
                "montant_total__sum"
            ]
            or 0,
            "montant_facture": Affaire.objects.aggregate(Sum("montant_facture"))[
                "montant_facture__sum"
            ]
            or 0,
            "montant_paye": Affaire.objects.aggregate(Sum("montant_paye"))[
                "montant_paye__sum"
            ]
            or 0,
        }
    ...
    def export_pdf(self, request, pk=None):
        ...
        p.drawString(
            50,
            740,
            f"Statut : {dict(Affaire.STATUT_CHOICES).get(affaire.statut, affaire.statut)}",
        )
        p.drawString(
            50, 720, f"Date de début : {affaire.date_debut.strftime('%d/%m/%Y')}"
        )
        if affaire.date_fin_prevue:
            p.drawString(
                50,
                700,
                f"Date de fin prévue : {affaire.date_fin_prevue.strftime('%d/%m/%Y')}",
            )
        ...
        buffer.seek(0)
        return FileResponse(
            buffer, as_attachment=True, filename=f"affaire_{affaire.reference}.pdf"
        )
